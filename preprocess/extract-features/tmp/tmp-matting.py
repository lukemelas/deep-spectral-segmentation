...
# Load
features_dict = torch.load('../features_VOC2007/VOC2007-image-features-dino_vits16-00091.pth')
segments_dict = torch.load('../eigensegments_VOC2007/VOC2007-dino_vits16-eigensegments-00091.pth')
...
# Calculate first eigenvector, remove sign ambiguity
K = 1
threshold = 0.0
eigenvalues, eigenvectors = eigsh(W_semantic, k=K, sigma=None, which='LM')
eigensegment = (eigenvectors[:, -1] > threshold).astype(float)
if 0.5 < np.mean(eigensegment > threshold).item() < 1.0:  # reverse segment
    eigensegment = (1 - eigensegment)

for k in range(K):
    print(f'Affinity {k} ({eigenvalues[k]:.1f}):')
    plt.imshow(eigensegment.reshape(H_, W_))
    plt.show()
...
def perform_matting(img: np.array, patch_mask: np.array, L = None):
    ...
    # Create trimap
    r = max(min(int(np.sum(eigensegment) // 50), 3), 1)
...
alpha = perform_matting(img=(img_np / 255), patch_mask=eigensegment)
plt.imshow(alpha)
plt.show()
...
evec = eigenvectors[:, -1].reshape(H_,W_)
evec = (evec - evec.min()) / (evec.max() - evec.min())
